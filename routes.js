import { getCurrentImageStream } from'./service/stream/index.js';
import { getCameraUrlByName } from"./service/camera/index.js";
...
const mqttOptions = {
  port: 1883,
  username: "user1", // Your MQTT username
  password: "1234", // Your MQTT password
};
const client = mqtt.connect("mqtt://192.168.1.7", mqttOptions);

client.on("connect", () => {
  console.log("Connected to MQTT broker");

  // Subscribe to a topic
  client.subscribe("camera.ui/motion", (err) => {
    if (err) {
...
client.on("message", (topic, message) => {
  ...
  const { camera, type, event } = JSON.parse(message.toString());
  // if (camera === "home" && type === "motion" && event === "videoanalysis") {
  if (1) {
    const cameraName = "boudoir";
    getCurrentImageStream(getCameraUrlByName(cameraName)).then((image) => {
      const date = new Date().toLocaleString();
      bot.sendMessage(channelId, `Phát hiện chuyển động từ camera ${cameraName} vào lúc ${date}`);
      bot
        .sendPhoto(channelId, image)
        .then((sent) => {
...
export default function (app, config) {
  ...
  app.get("/:stream", function (req, res, next) {
    ...
    if (streams.indexOf(streamName) !== -1) {
      console.log("received request for image from stream '" + streamName + "'");
      ...
      let resolution =
        config.ffmpeg && config.ffmpeg.resolution
          ? config.ffmpeg.resolution
          : "1920x1080"; // native resolutions are 1920x1080, 1024x576 or 640x360
      let quality =
        config.ffmpeg && config.ffmpeg.quality
          ? Number(config.ffmpeg.quality)
          : 5;
          console.log(
        "for stream '" +
          streamName +
      ...
      stream.on("start", () => {
        console.log("start event received for stream '" + streamName + "'");
      }); // start

      stream.on("stop", () => {
        console.log("stop stream '" + streamName + "'");
        return "stop";
      }); // start

      stream.on("data", function (chunk) {
        // if we don't yet have a frame to process then proceed...
        console.log(haveFrame);
        if (haveFrame == false) {
          ...
          console.log(
            "frame received, length " + _formatNumber(chunk.length) + " bytes"
          );
          ...
          bot
            .sendPhoto(channelId, Buffer.from(chunk))
            .then((sent) => {
              console.log("Image sent successfully!");
            })
            .catch((error) => {
          ...
          console.log("frame sent, exiting event handler");
          console.log("------");
          stream.stop();
          return;
        }
